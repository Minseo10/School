...
int main() {
	...
	output = UnsharpMask(input, 5, 5, 5, "zero-paddle",0.5); //Boundary process: zero-paddle, mirroring, adjustkernel
...
//gaussian filter function with color image
//parameters
...
Mat gaussianfilter(const Mat input, int n, float sigmaT, float sigmaS, const char* opt) {
	...
	//R,G,B 각각의 intensity에 대해 filtering을 해준다.
	for (int i = 0; i < row; i++) {
		for (int j = 0; j < col; j++) {
				...
				for (int a = -n; a <= n; a++) {
					for (int b = -n; b <= n; b++) {
						...
						if ((i + a <= row - 1) && (i + a >= 0) && (j + b <= col - 1) && (j + b >= 0)) { //if the pixel is not a border pixel
							sum1_r += kernelvalue * (float)(input.at<C>(i + a, j + b)[0]); //0:R, 1:G, 2:B
...
//unsharp masking function with color image
//parameters
//input: intensity matrix of input image , n: filter kernel size is (2n+1)(2n+1) , sigmaT/sigmaS: standard deviation for x/y coordinate
...
Mat UnsharpMask(const Mat input, int n, float sigmaT, float sigmaS, const char* opt, float k) {
	...
	L = gaussianfilter(input, n, sigmaT, sigmaS, opt); //Low-pass filtering with gaussian filter(color image)
