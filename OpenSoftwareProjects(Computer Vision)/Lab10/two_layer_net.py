...
#max: array, matrix에서 최대 원소 리턴 / maximum: 2개 파라미터 비교 후 최대값 리턴
def rel_error(x, y):
...
best_val = -1 #validation accuracy of best network
best_stats = None
...
for lr in learning_rates:
    for rs in regularization_strengths:
        ...
        # if the validation accuracy is largest, then it is the best network
        if best_val < acc_val:
            best_stats = stats
            best_val = acc_val
            best_net = net
