...
int max_serv_time = 20; // maximum service time for one customer
int clock;
...
void main()
{
	...
	while (clock < duration) {
		...
		if (service_time1 > 0 && service_time2 == 0) { //only first bank staff is serving a customer
			service_time1--;
			service_time2 = remove_customer();
		}

		if (service_time1 == 0 && service_time2 > 0) { //only second bank staff is serving a customer
			service_time2--;
