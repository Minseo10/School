...
int main() {
	...
	Mat input = imread("C:\\Users\\MIN\\Desktop\\권민서\\학교\\2022-1\\오소프\\Lab06\\writing.jpg", CV_LOAD_IMAGE_COLOR);
	Mat input_gray;
	Mat output;
...
//input: intensity matrix of input image, n: filter kernel size if (2n+1)(2n+1), bnumber: weight
Mat adaptive_thres(const Mat input, int n, float bnumber) {
	...
	// Initialiazing Kernel Matrix (uniform mean filter kernel)
	...
	for (int i = 0; i < row; i++) { //for each pixel in the output
		for (int j = 0; j < col; j++) {
			...
			// finds the mean intensity using uniform mean filtering with zero paddle border process.
			float sum1 = 0.0;
			...
			float temp = bnumber * (G)sum1; //threshold for each pixel: bnumber*m(i,j)
